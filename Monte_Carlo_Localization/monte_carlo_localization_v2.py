...
class ParticleFilter(object):
    ...
    def get_general_random_free_state(self):
        while True:
            # Note: we initialize particles closer to the robot's initial
            # position in order to make the initialization easier
    ...
    def handle_observation(self, laser_scan, dt):
        ...
        N_eff = 0
        exp_errors = np.sum( [np.exp(-x) for x in errors] )

        for i in range(len(self.particles)):
            temp = np.exp(-errors[i]) / exp_errors
    ...
    def resample(self):
        ...
        idx = int(u * (len(self.particles)-1))
        beta = 0
        ...
        for p in self.particles:
            ...
            while beta > self.weights[idx]:
                ...
                idx = (idx + 1) % len(self.particles)

            p_new = copy.copy(self.particles[idx])
            temp_particles.append(p_new)




        # Proportion of the particles to keep as resampled. The rest will be
        # generated randomly. Random particles are disabled as of submission of this code. 
        noRandomize = 1

        temp_particles_length = len(temp_particles)
        reused_particles_length = int(len(temp_particles)* noRandomize)

        

        new_particles = [p for p in temp_particles]


        for i in xrange(temp_particles_length-reused_particles_length):
            xrand, yrand, theta_or_something = self.get_general_random_free_state()
            new_particles.append(Particle(i, xrand, yrand, theta_or_something))


        #print(type(temp_particles[0]))
        #print(type(new_particles[0]))
        #print "TEMP:", temp_particles
    ...
    def get_prediction_error_squared(self, laser_scan_msg, particle):
        ...
        subsampled_ranges, subsampled_angles = self.subsample_laser_scan(laser_scan_msg)
        
        # TODO: simulate a laser scan using one of the methods of this class
        
        sim_ranges = self.simulate_laser_scan_for_particle(particle.x, particle.y, particle.theta, subsampled_angles, self.laser_min_range, self.laser_max_range )
        ...
        for i in range(len(sim_ranges)):
            norm_error += (sim_ranges[i] - subsampled_ranges[i])**2
