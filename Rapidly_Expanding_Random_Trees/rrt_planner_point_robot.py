...
def rrt_search(G, tx, ty):
    ...
    for i in range(1, 50000):
        ...
        closestIndex = closestPointToPoint(G, qrand)
        qnear = vertices[closestIndex]


        [lx, ly] = lineFromPoints(qnear, qrand)
        l = pointPointDistance(qnear, qrand)

        # make line of same direction but length SMALLSTEP
        shortline = [lx * SMALLSTEP, ly * SMALLSTEP]
        qstep = [qnear[0] + shortline[0], qnear[1] + shortline[1]]

        #print qrand, '...........', qnear, '\n', vertices
        #print lx, ly, math.sqrt(lx*lx+ly*ly), l, shortline

        for j in range(len(obstacles)):
            if  lineHitsRect(qnear, qstep, obstacles[j]) or inRect(qstep, obstacles[j], 2): #if line not obstructed
                isFree = False
        if isFree:
            if (int(qstep[0]) in range(2, 960)) and (int(qstep[1]) in range(2, 700)):
                newestIndex = pointToVertex(qstep)
                G[nodes].append(newestIndex)
                G[edges].append((closestIndex, newestIndex))
                canvas.polyline([qnear, qstep])

        if (int(qstep[0]) in range(tx - SMALLSTEP, tx + SMALLSTEP)) and (int(qstep[1]) in range(ty - SMALLSTEP, ty + SMALLSTEP)):
    ...
    parentIndex = G[nodes][-1]
    pathLength = 0
    if (int(qstep[0]) in range(tx - SMALLSTEP, tx + SMALLSTEP)) and (int(qstep[1]) in range(ty - SMALLSTEP, ty + SMALLSTEP)):
        while parentIndex != 0:
            currentIndex = parentIndex
            parentIndex = returnParent(currentIndex)
            pathLength = pathLength + SMALLSTEP
        print "Path found."
    else:
        print "No path found."
    return [i, pathLength]
...
for i in range(4, 5):
    ...
    for j in range(0, 1):
        ...
        if visualize: canvas.markit(start_x, start_y, r=SMALLSTEP)

        [newCount, newLength] = rrt_search(G, tx, ty)
        iterAcc += newCount
        newLength += newLength

    mean_path.append(newLength)
    mean_iteration.append(iterAcc/10)


print "step sizes:", step_sizes
print "mean path:", mean_path
print "mean iteration", mean_iteration
plt.figure(4)
x = step_sizes
y = mean_path
